...
if( ! class_exists( 'Edulab_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Edulab_Control_Repeater extends WP_Customize_Control {
        ...
        public function __construct( $manager, $id, $args = array() ) {
            ...
            if ( empty( $this->button_label ) ) {
                $this->button_label = sprintf( esc_attr__( 'Add new %s', 'edulab' ), $this->row_label['value'] );
            }
                ...
                foreach ( $this->fields as $field ) {
                    if ( isset( $field['type'] ) ){
                        if( 'color' === $field['type'] ){ 
                            ...
                            wp_enqueue_script( 'all', get_template_directory_uri(). '/js/all.js', array(), '5.2.0' );
                            wp_enqueue_script( 'v4-shims', get_template_directory_uri(). '/js/v4-shims.js', array(), '5.2.0' );
                        }
                    }
                }
            }
            wp_enqueue_script( 'labtheme-repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
            wp_enqueue_style( 'labtheme-repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.css', null );
        }
        ...
        protected function render_content() {
            ...
            <?php if ( isset( $this->choices['limit'] ) ) : ?>
                <p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'edulab' ), esc_html( $this->choices['limit'] ) ); ?></p>
            <?php endif; ?>
            ...
            <script type="text/html" class="customize-control-repeater-content">
                ...
                <li class="repeater-row minimized" data-row="{{{ index }}}">
                    ...
                    <div class="repeater-row-content">
                        <# _.each( data, function( field, i ) { #>
                            ...
                            <div class="repeater-field repeater-field-{{{ field.type }}}">
                                ...
                                <# } else if ( 'color' === field.type ) { #>
                                    ...
                                    <label>
                                        ...
                                        <# if ( field.description ) { #>
                                            <span class="description customize-control-description">{{{ field.description }}}</span>
                                        <# } #>
                                        <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'edulab' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
                                ...
                                <# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
                                    ...
                                    <figure class="labtheme-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'edulab' ); ?>" >
                                        <# if ( field.default ) { #>
                                            <# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
                                            <img src="{{{ defaultImageURL }}}">
                                        <# } else { #>
                                            <?php esc_attr_e( 'No Image Selected', 'edulab' ); ?>
                                        <# } #>
                                    </figure>
    
                                    <div class="actions">
                                        <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'edulab' ); ?></button>
                                        <button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'edulab' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'edulab' ); ?>" >
                                            <# if ( field.default ) { #>
                                                <?php esc_attr_e( 'Change Image', 'edulab' ); ?>
                                            <# } else { #>
                                                <?php esc_attr_e( 'Add Image', 'edulab' ); ?>
                                            <# } #>
                                        </button>
                                        <# if ( field.default.id ) { #>
                                ...
                                <# } else if ( field.type === 'upload' ) { #>
                                    ...
                                    <figure class="labtheme-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'edulab' ); ?>" >
                                        <# if ( field.default ) { #>
                                        ...
                                        <# } else { #>
                                            <?php esc_attr_e( 'No File Selected', 'edulab' ); ?>
                                        <# } #>
                                    </figure>
    
                                    <div class="actions">
                                        <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
                                        <button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'edulab' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'edulab' ); ?>" >
                                            <# if ( field.default ) { #>
                                                <?php esc_attr_e( 'Change File', 'edulab' ); ?>
                                            <# } else { #>
                                                <?php esc_attr_e( 'Add File', 'edulab' ); ?>
                                            <# } #>
                                        </button>
                                        <# if ( field.default.id ) { #>
                                            <input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
                        ...
                        <# }); #>
                        <button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'edulab' ); ?></button>
                    </div>
                </li>
            </script>
            <?php
        ...
        function get_icon_list(){
            $edulab_icons = include wp_normalize_path( get_template_directory() . '/inc/fontawesome.php' ); 
            if( $edulab_icons ){ ?>
            <div class="font-awesome-list-template">
                <div class="font-awesome-list">
                    <ul class="font-group">
                    <?php
                        foreach( $edulab_icons as $font ){
